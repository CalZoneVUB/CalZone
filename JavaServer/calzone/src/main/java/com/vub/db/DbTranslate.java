...
// CHECK
	...
	public static boolean checkIfUserNameAvailable(String username){
		String sql = "SELECT 1 FROM Users WHERE Username = '"+ username +"'	"+
					 "UNION	"+
					 "SELECT 1 FROM NotRegisteredUsers WHERE Username = '"+ username +"';";
...
// UPGRADE NotRegisteredUser to User
	...
	public static void upgradeNotRegisteredUser(User user){
		// 1) make a new row in Users with the info from user and the PersonID associated to that username 
		// 2) remove row in NotRegisteredUsers
		// *) remove in ActivationKeys? or refer the activation key to User ?
		
		// UPDATE Users
		// SET Users.PersonID = (SELECT PersonID FROM NotRegisteredUsers WHERE Username = 'test')
		// WHERE Username = 'test'
		
		DbLink.executeSql("INSERT INTO Users (Username, Password, PersonID)" +
						  " VALUES('" + user.getUserName() + "', '" + user.getPassword() +"', (SELECT PersonID FROM NotRegisteredUsers WHERE Username = '" + user.getUserName() + "'));");
		DbLink.executeSql("DELETE FROM NotRegisteredUsers WHERE Username = '" + user.getUserName() + "';");
	}
	
// DELETE
	
	public static void deleteActivationKey(ActivationKey activationKey){
		DbLink.executeSql("DELETE FROM ActivationKeys WHERE KeyString = '" + activationKey.getKeyString() + "';");
	}
	
// INSERT
	
	public static void insertActivationKey(ActivationKey activationKey){
		DbLink.executeSql("INSERT INTO ActivationKeys (`KeyString`, `CreatedOn`, `NotRegisteredUserID`) VALUES ('" + activationKey.getKeyString() + "', '"+ activationKey.getCreatedOn() +"' , (SELECT NotRegisteredUserID FROM NotRegisteredUsers WHERE Username = '" + activationKey.getUserName() + "'));");
	}
...
//		COMMIT;
		...
		String sql1 = "INSERT INTO Persons (LastName, FirstName, Email, Birthdate)" +
			   	 	  " VALUES ('" + user.getLastName() + "', '" + user.getFirstName() + "', '" + user.getEmail() + "', '" + user.getBirthdate() +"'); ";
		String sql2 = "INSERT INTO NotRegisteredUsers (Username, Password, PersonID)" +
			   	 	  " VALUES('" + user.getUserName() + "', '" + user.getPassword() + "', LAST_INSERT_ID());";
...
// UPDATE
	...
	public static void updateUser(User user){ 
		DbLink.executeSql("UPDATE Users" +
						  " SET Password = '" + user.getPassword() + "', Language = '" + user.getLanguage() + "'" +
...
// SELECT	
	...
	public static ActivationKey selectUserByActivationKey(String keyString){
		...
		rs = DbLink.executeSqlQuery("SELECT ActivationKeys.CreatedOn, NotRegisteredUsers.Username"+
									" FROM ActivationKeys"+
									" JOIN NotRegisteredUsers ON ActivationKeys.NotRegisteredUserID = NotRegisteredUsers.NotRegisteredUserID"+
									" WHERE ActivationKeys.KeyString = '"+ keyString +"';");
	...
	public static User selectNotRegisteredUserByUsername(String username){
		...
		rs = DbLink.executeSqlQuery("SELECT NotRegisteredUsers.Password, Persons.LastName, Persons.FirstName, Persons.Email, Persons.BirthDate" +
		  							" FROM Persons" +
		...
		try {
			if (!rs.isBeforeFirst()){
				System.out.println("-> ! This username doesn't exist in the database !");
				return null;
			}
			else {
				rs.next();
	...
	public static List<User> selectAllUsers(){
		...
		rs = DbLink.executeSqlQuery("SELECT Users.Username, Users.Password, Users.Language, UserTypes.UserTypeName, Persons.LastName, Persons.FirstName, Persons.Email, Persons.BirthDate" +
		  							" FROM Persons" +
	...
	public static User selectUserByEmail(String email){
		...
		rs = DbLink.executeSqlQuery("SELECT Users.Username, Users.Password, Users.Language, UserTypes.UserTypeName, Persons.LastName, Persons.FirstName, Persons.BirthDate" +
		  							" FROM Persons" +
		  							" JOIN Users ON Persons.PersonID = Users.PersonID" +
		  							" JOIN UserTypes ON Users.UserTypeID = UserTypes.UserTypeID" +
	...
	public static User selectUserByUsername(String username){
		...
		rs = DbLink.executeSqlQuery("SELECT Users.Password, Users.Language, UserTypes.UserTypeName, Persons.LastName, Persons.FirstName, Persons.Email, Persons.BirthDate" +
		  							" FROM Persons" +
		  							" JOIN Users ON Persons.PersonID = Users.PersonID" +
		  							" JOIN UserTypes ON Users.UserTypeID = UserTypes.UserTypeID" +
